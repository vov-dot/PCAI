...
- Умная очистка контента: удаление рекламы, навигации, мусора
- Автоматический фоллбэк на сниппеты при ошибках загрузки страниц
- Переключаемая галочка **🔍 Поиск** (влияет на чат + Docs/Sheets)
...
### 🎨 Интерфейс
- Тёмная тема в стиле **ChatGPT / Qwen**
- Адаптивный дизайн (мобильные + десктоп)
- Боковая панель с историей чатов
- Модальные окна для создания документов
...
4. В файле `base.py` проверьте название модели:
   ```python
   model = lms.llm("qwen/qwen3-8b")  # должно совпадать с загруженной
   ```
...
### 🔍 Чат с поиском
1. Нажмите галочку **🔍 Поиск** в панели инструментов
...
```
В `google_api.py` используйте `range='A1'` вместо `'Sheet1!A1'`.
